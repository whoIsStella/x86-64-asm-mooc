...
## Why learn this, and how the parts connect

**Why should you care?** Not because you'll write programs in assembly — you almost
never will. You learn it because it removes the magic from everything above it. After
this course, a pointer is just an address in a register; a struct is just bytes at an
offset; a function call is just "arguments in registers, jump, result in `rax`"; a
crash is just `rsp` pointing somewhere it shouldn't. You will read compiler output,
debug an optimized binary, reason about performance, and understand security bugs —
because you can see the machine the way the CPU does. Assembly is the ground floor that
makes every higher floor make sense.

**How does it all connect?** Each part adds one layer, and every later part stands on
the earlier ones:

```text
Part 0   What assembly IS — machine code, registers, one instruction.   the vocabulary
Part 1   A function: arguments in, result out.                          the contract
Part 2   Reach memory; compute addresses (lea).                         data lives in RAM
Part 3   Compute, and the flags every operation leaves behind.          arithmetic + flags
Part 4   Act on those flags: branches and loops.                        control flow
Part 5   Call other functions safely: the stack and the ABI.           composition
   └─ With Parts 0–5 you can write any correct C-callable function. The rest is reach and depth:
Part 6   Static data and position-independent addressing.               tables, globals
Part 7   One-instruction memory loops (string/REP).                     bulk memory ops
Part 8   One instruction, many values (SIMD).                           data parallelism
Part 9   Talk to the kernel directly (syscalls, no libc).               the OS boundary
Part 10  Drop assembly into C (inline asm).                             the C interop seam
Part 11  Read it back: debugging & disassembly.                         seeing the machine
Part 12  Capstones — combine everything into real programs.             synthesis
```

Read it as a spiral: Part 2's `[base + index*scale]` reappears in Part 4's array loop,
Part 6's tables, and Part 8's vector loads. Part 3's flags drive Part 4's branches,
which structure every loop after. Part 5's ABI is the contract every other part quietly
obeys. By Part 12 you are not learning new tricks — you are assembling ones you already
own. Each part lecture opens with a one-line **Why care / how it connects** so you never
lose that thread.

| # | Part | Prereq | Est. | Status |
|---|------|--------|------|--------|
| 0 | Ground Zero: what assembly is (no prior knowledge) | — | 1h | ✅ |
